...
export default function Lightbox({ heroImagePath, lightbox, setLightbox }: { heroImagePath: string, lightbox: boolean, setLightbox: Dispatch<SetStateAction<boolean>> }) {
  ...
  return (
    <div className={`lightbox z-20 fixed top-0 left-0 w-full h-full bg-[hsl(0deg_0%_0%_/_90%)] transition-opacity duration-500 ${lightbox === true ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0 delay-500'}`}>
      ...
      <div className='image-container absolute w-[800px] max-w-full max-h-svh top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-col gap-5 items-end p-5 sm:p-10'>
        ...
        <img src={`..${heroImagePath}`} className={`${lightbox === true ? 'opacity-1 translate-y-0' : 'opacity-0 translate-y-4'} w-full h-full object-contain max-h-[calc(100vh-200px)] transition-all delay-300`} alt="" />
